...
\institute{Universidade Federal do Rio de Janeiro - Centro de Tecnologia}
\iaddress{Av. Athos da Silveira Ramos, 149 - Cidade Universitária, Ilha do Fundão, Rio de Janeiro - RJ }

\author{
\name{Abraão Carvalho Gomes}
\email{abraao.cg@poli.ufrj.br}
}

\author{
\name{Giovanni Paes}
\email{giovanni\_paesleme@poli.ufrj.br}
}

\author{
\name{Rafael Lúcio}
\email{rafael.martins@poli.ufrj.br}
}

\vspace{8mm}
\begin{resumo}
Avaliar se um jogador deveria ou não perder a posse sob pressão é central na análise tática do futebol. Propomos o \emph{xR} (Recuperação Esperada), a probabilidade calibrada de a equipe que pressiona recuperar a bola, estimada a partir do StatsBomb-360. Adotando o alvo com restrição espacial já presente na literatura (recuperar em até 5\,s e a até 5\,m), mostramos uma relação \textbf{dose-resposta}: a informatividade da geometria \emph{cresce} conforme o alvo é localizado (AUC só-geometria de 0,59 a 0,68), e \textbf{nenhuma arquitetura} (GNN, CNN, Set Transformer) supera um alvo permissivo ($\approx$0,59). Explicamos o mecanismo: 76\% das recuperações em até 10\,s ocorrem longe/tarde demais (mediana 13\,m) para serem causadas pela pressão. Uma regressão logística \textbf{leve e calibrada} (ECE 0,003) iguala modelos pesados e habilita usos agregados: análise de adversário, treino, \emph{scouting} e revisão pós-jogo (xR \emph{vs.} realizado). Delimitamos honestamente o escopo: \emph{ratings} individuais de jogador têm confiabilidade $\approx$0,09, inadequados para veredito individual.
\end{resumo}
...
\begin{abstract}
Assessing whether a player should or should not lose possession under pressure is central to tactical football analysis. We propose \emph{xR} (Expected Recovery), the calibrated probability that the pressing team regains the ball, estimated from StatsBomb-360 data. Adopting the spatially-restricted target already present in the literature (recovery within 5\,s and 5\,m), we show a \textbf{dose-response} relationship: the informativeness of player geometry \emph{grows} as the target is localized (geometry-only AUC from 0.59 to 0.68), and \textbf{no architecture} (GNN, CNN, Set Transformer) beats a loose target ($\approx$0.59). We explain the mechanism: 76\% of within-10\,s recoveries occur too far or too late (median 13\,m) to be caused by the press. A \textbf{light, calibrated} logistic regression (ECE 0.003) matches heavy models and enables aggregate uses: opponent analysis, training, scouting and post-match review (xR \emph{vs.} actual). We honestly delimit scope: individual player ratings have reliability $\approx$0.09, unfit for individual verdicts.
\end{abstract}
...
\begin{enumerate}
\item \textbf{Mecanismo: a geometria é mais informativa sob alvos locais.} Adotando o alvo com restrição espacial da literatura, mostramos uma relação \emph{dose-resposta} --- a AUC só-geometria cresce monotonicamente conforme o alvo localiza (0,59 $\to$ 0,68) --- e explicamos por quê: \textbf{76\%} das recuperações em $\le$10\,s são causalmente desconexas da pressão. Nenhuma arquitetura (GNN, CNN, Set Transformer) supera o alvo permissivo ($\approx$0,59).
...
A valoração de pressing foi formalizada por \citet{robberechts2019} (VPEP) num arcabouço de risco--recompensa, estendido por \citet{merckx2021} com detecção baseada em regras e modelagem de desfechos. \citet{lee2025express} (exPress) propõem um modelo XGBoost sobre dados de eventos e StatsBomb-360 para estimar a probabilidade de recuperação, com aplicação a ajustes posicionais contrafactuais; reportam AUC de 0,607 (recuperação em 5\,s), até 0,712 sob ``2 ações'' e \textbf{exploram alvos com restrição espacial} (recuperação num raio de 3/5/9\,m). Essa restrição espaço-temporal ecoa convenções táticas --- o raio de pressão do StatsBomb ($\approx$4--5\,m) e a ``regra dos 5--6\,s'' da contrapressão. Modelos de superfície como o SoccerMap \citep{fernandez2020soccermap} usam CNNs sobre representações espaciais. A geometria do pressing foi estudada por \citet{andrienko2017} e, via pose 3D, por \citet{gu2024pressure}; a contrapressão por \citet{bauer2021counterpress}.

Em valoração de ações e posse, VAEP \citep{decroos2019vaep} e \emph{Expected Threat} \citep{singh2019xt} atribuem valor a ações a partir do contexto, e modelos de \emph{Expected Possession Value} foram desenvolvidos no basquete \citep{cervone2016epv} e em outros esportes coletivos. A importância de probabilidades \emph{calibradas} é estabelecida no aprendizado de máquina \citep{niculescu2005calibration}; no futebol, modelos de \emph{expected goals} são validados por calibração e usados em agregado \citep{anzer2021xg} --- prática que aqui aplicamos ao \emph{pressing}. Nosso trabalho difere por (i) isolar o efeito da definição do alvo sobre o sinal geométrico e (ii) auditar a confiabilidade dos \emph{ratings} individuais, em vez de apresentá-los sem validação.
...
\subsection{O alvo e o sinal geométrico: uma relação dose-resposta}
O alvo com restrição espacial (recuperar em $\le$5\,s e a $\le$5\,m) foi proposto por \citet{lee2025express} e ecoa convenções táticas (raio de pressão; regra dos 5--6\,s). Nossa contribuição não é o alvo, e sim \textbf{mostrar e explicar} por que ele funciona. A Figura~\ref{fig:ladder} revela uma \textbf{relação dose-resposta} ao longo dos \textbf{eixos naturais de localidade} do alvo --- \emph{tempo} (10\,s $\to$ 5\,s) e \emph{espaço} (sem raio $\to$ 9\,m $\to$ 5\,m), apertando \emph{uma} dimensão por passo: a \emph{AUC só-geometria} \textbf{cresce monotonicamente} de 0,586 a 0,679 (todas as variáveis, de 0,591 a 0,702). Um alvo baseado em \emph{sequência} (recuperar em $\le$2 ações) --- eixo distinto, sem teto direto de metros ou segundos --- também cai sobre o gradiente (só-geometria 0,637) e por isso é exibido \textbf{à parte} na figura: corrobora que o efeito independe de \emph{como} se mede a localidade. O mecanismo é a remoção de \textbf{ruído de rótulo}: \textbf{76\% das recuperações em $\le$10\,s ocorrem a $>$5\,m ou $>$5\,s} do ponto de pressão (mediana 13\,m), dificilmente causadas por aquela pressão; localizar o alvo as descarta e \emph{recupera} o sinal geométrico real.

\begin{figure}[ht]
\centering
\includegraphics[width=0.74\textwidth]{figs/fig_ladder.png}
\caption{Relação dose-resposta ao longo dos eixos de tempo e espaço (linha principal); o alvo por sequência ($\le$2 ações), de eixo distinto, é mostrado à parte como ponto corroborativo. O ganho vem do alvo, não da arquitetura (Tabela~\ref{tab:plateau}).}
\label{fig:ladder}
\end{figure}
...
A Figura~\ref{fig:cal} mostra que o xR é bem calibrado: quando o modelo prevê risco de 20\%, observa-se $\approx$20\%. Essa propriedade legitima o xR como um \emph{expected model}, na mesma linhagem do xG \citep{anzer2021xg}. A Figura~\ref{fig:lift} traduz a discriminação em valor operacional: revisando as 10\% jogadas de maior risco, encontra-se densidade de perdas $2{,}3\times$ maior que o acaso, capturando $\approx$23\% de todas as perdas --- útil para \textbf{triagem de vídeo}, não para alarme automático.
...
\subsection{Contexto do evento: o tipo de gatilho adiciona sinal?}
Uma análise complementar caracterizou \emph{gatilhos de pressão} pelo passe adversário que precede a pressão (recuo, passe lateral, passe perto da linha lateral, cobrança de lateral) e pela situação do portador (isolado, sem companheiro a $\le$15\,m). Avaliamos, na Euro 2020 (13.999 pressões), se o tipo de gatilho \textbf{acrescenta sinal} ao modelo logístico, com um baseline \emph{honesto} (todas as pressões) e o alvo \texttt{y\_5s\_5m}.

\textbf{Descritivamente}, os gatilhos diferem bastante (Tabela~\ref{tab:trig}): pressionar após um \emph{recuo} recupera muito menos (razão 0,30 sobre o baseline), enquanto após \emph{cobrança de lateral} recupera mais (1,73). \textbf{Como sinal preditivo, porém, o tipo de gatilho quase nada acrescenta:} incluir os indicadores de gatilho na logística altera a AUC de 0,702 para 0,694 sob \texttt{y\_5s\_5m} ($\Delta=-0{,}008$) e de 0,599 para 0,608 sob \texttt{y\_5s} ($+0{,}009$). Ou seja, sob o alvo estrito a \textbf{geometria já captura o sinal}, e a categoria do gatilho é \emph{descritiva/explicativa}, não incrementalmente preditiva --- coerente com a tese de que o ganho vem do alvo e da configuração espacial, não de mais contexto categórico.

\begin{table}[ht]
\centering
\caption{Recuperação por gatilho na Euro 2020 (alvo \texttt{y\_5s\_5m}; baseline = todas as pressões, 6,1\%).}
\label{tab:trig}
\begin{tabular}{lccc}
\toprule
...
\subsection{Contrapressão: a vantagem persiste sob o alvo estrito?}
Outra análise complementar isolou a \emph{contrapressão} --- pressão até 5\,s após a própria equipe perder a bola --- e comparou sua eficácia à da pressão regular no terço ofensivo. Sob o alvo permissivo (recuperação em $\le$10\,s), a contrapressão recupera mais (29,5\% \emph{vs.}\ 24,0\%; $p<0{,}001$), confirmando a literatura. Reavaliamos sob o alvo estrito \texttt{y\_5s\_5m}: na comparação \emph{bruta} a diferença deixa de ser significativa (6,7\% \emph{vs.}\ 6,1\%; $p=0{,}14$), \textbf{mas} isso é um artefato de confundimento por zona --- a contrapressão concentra-se no terço ofensivo, onde a recuperação imediata-e-local é mais rara. Controlando a zona numa logística, a contrapressão volta a ter efeito positivo e significativo (\emph{odds ratio} \textbf{1,36}; IC\,95\% $[1{,}18;\,1{,}56]$). \textbf{Conclusão:} o benefício da contrapressão é real mesmo no alvo estrito, porém menor e \emph{dependente de zona} --- mais um caso em que a definição do alvo e o contexto espacial determinam a leitura.
...
\textbf{Exemplos concretos.} Para ilustrar com rigor, treinamos a logística em 36 jogos da Euro 2020 e aplicamos o xR a 15 jogos de \emph{teste} que o modelo nunca viu (taxa-base 5,5\%). Em todos os casos, a \emph{predição} é o xR de cada pressão --- a probabilidade de a equipe que pressiona recuperar a bola em $\le$5\,s e a $\le$5\,m.

\emph{Análise de adversário (pré-jogo).} À pergunta \emph{``Vamos enfrentar a Itália --- em que zona a nossa pressão tem maior chance de recuperar a bola?''}, o modelo pontua as 591 pressões sofridas pela Itália e agrega o xR por zona: a pressão rende mais no \textbf{terço defensivo} dela (xR médio 8,4\%) do que no de ataque (3,8\%) --- onde priorizar o gatilho. O padrão se repete contra Suíça (7,7\% \emph{vs.}\ 3,5\%) e Espanha (9,4\% \emph{vs.}\ 4,3\%).

\emph{Revisão pós-jogo (xR esperado \emph{vs.}\ realizado, Figura~\ref{fig:usecase}).} À pergunta \emph{``No jogo França$\times$Suíça, a pressão da Suíça rendeu o que se esperava?''}, o modelo prevê o xR das 220 pressões da Suíça e os soma: \textbf{13,9 recuperações esperadas} contra \textbf{11 realizadas} ($-2{,}9$) --- a equipe pressionou bem, mas converteu \emph{abaixo} do esperado. As pressões mais instrutivas para o vídeo são as de \textbf{xR alto que falharam}: situações em que a geometria \emph{favorecia} a recuperação (xR 18--21\%) e, ainda assim, a bola não foi retomada. São, por definição, as melhores oportunidades perdidas --- revisá-las mostra \emph{o que deu errado apesar de um bom posicionamento}, e por isso viram os primeiros \emph{clipes} de revisão. A diferença esperado$-$realizado de um \emph{único} jogo é direcional e sujeita a ruído amostral; ela só vira veredito confiável ao longo de uma temporada --- os \emph{clipes} de maior xR são o resultado robusto por jogo.

\emph{Triagem de vídeo / treino.} À pergunta \emph{``Temos pouco tempo --- o que revisar primeiro?''}, ordenam-se todas as pressões pelo xR. Como o xR \emph{concentra} as recuperações, revisar apenas a fração de maior xR cobre a maior parte delas com pouco vídeo: as \textbf{10\% de maior xR} têm densidade de recuperação \textbf{2,9$\times$} a taxa-base e contêm \textbf{28\%} de todas as recuperações do conjunto; subindo para 20\% do vídeo, captura-se 44\%. É a tradução direta do poder de \emph{ranqueamento} do modelo em economia de análise. As Figuras~\ref{fig:usecase} e~\ref{fig:usecase2} trazem o passo a passo da revisão pós-jogo em dois exemplos e dois protocolos de validação (36 jogos de treino/15 de teste e 47/4), ilustrando que a história não depende do tamanho do \emph{held-out}.

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{figs/fig_usecase.png}
\caption{Passo a passo de um exemplo real (\textbf{36 jogos de treino, 15 de teste} \emph{held-out}): da pergunta da comissão à resposta do xR, na revisão pós-jogo de França$\times$Suíça.}
\label{fig:usecase}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{figs/fig_usecase2.png}
\caption{O mesmo passo a passo com mais dados de treino (\textbf{47 jogos de treino, 4 de teste} \emph{held-out}): revisão pós-jogo de Itália$\times$Inglaterra. A leitura é a mesma, ilustrando que o fluxo não depende do tamanho do \emph{held-out}.}
\label{fig:usecase2}
\end{figure}
