...
\begin{frame}{Estudo de contrapressão}
  ...
  {\small Clara no alvo frouxo; no estrito \alert{some na comparação bruta} (confundimento por zona) e
...
\begin{frame}{Impacto da geometria (achado central)}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \centering\includegraphics[width=\linewidth]{figs/fig_target.png}\\
      {\scriptsize Alvo local (\,$\leq$5\,s e $\leq$5\,m\,): \alert{0,59 $\to$ 0,70}}
    \end{column}
    \begin{column}{0.5\textwidth}
      \centering\includegraphics[width=\linewidth]{figs/fig_ablation.png}\\
      {\scriptsize Geometria = maior bloco; \alert{mais informativa} sob o alvo estrito}
    \end{column}
  \end{columns}
  \vspace{1mm}
  \centering{\small Controle de \emph{base rate} (0,573) + \emph{bootstrap} (IC95\% [0,10;\,0,12]):
  o ganho é \alert{real}, não artefato.}
\end{frame}

% ---- 7. Conclusões ----
\begin{frame}{Conclusões}
  \begin{enumerate}
    \item[\textcolor{rxpurple}{\textbf{1.}}] \textbf{O alvo define o sinal.} Restringir no tempo \emph{e} no espaço eleva a AUC de
    0,59 para \alert{0,70} --- e torna a geometria mais informativa.
    \vspace{2mm}
